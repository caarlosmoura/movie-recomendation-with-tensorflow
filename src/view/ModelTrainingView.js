...
export class ModelView extends View {
    ...
    updateTrainingProgress(progress) {
        ...
        this.#trainModelBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Building index...';

        if (progress.progress === 100) {
            this.#trainModelBtn.disabled = false;
            this.#trainModelBtn.innerHTML = '<i class="bi bi-diagram-3"></i> Train Model - Build Qdrant Index';
        }
    }

    renderAllUsersPurchases(users) {
        const html = users.map(user => {
            const purchasesHtml = (user.watchHistory || []).map((movie) => {
                return `<span class="badge bg-light text-dark me-1 mb-1">${movie.title}</span>`;
            }).join('');
            ...
            return `
                <div class="user-purchase-summary">
                    ...
                    <div class="purchases-badges">
                        ${purchasesHtml || '<span class="text-muted">No watch history</span>'}
                    </div>
                </div>
            `;
        }).join('');
