export class UserController {
    ...
    async handlePurchaseAdded({ user, movie }) {
        ...
        updatedUser.watchHistory.push({
            ...movie
        });

        await this.#userService.updateUser(updatedUser);

        const lastPurchase = updatedUser.watchHistory[updatedUser.watchHistory.length - 1];
    ...
    async handlePurchaseRemove({ userId, product }) {
        const user = await this.#userService.getUserById(userId);
        const index = user.watchHistory.findIndex(item => item.id === product.id);
    ...
    async displayUserDetails(user) {
        ...
        this.#userView.renderPastPurchases(user.watchHistory);
